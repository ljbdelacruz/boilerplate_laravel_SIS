...
@section('content')
       <div id="page-meta" data-title="Add Section" data-parent="Section">
            ...
            <div class="bg-yellow-100 shadow-lg rounded-lg p-8">
                <form action="{{ route('sections.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="text-left">
                        ...
                        <input type="text" name="name" id="name" placeholder="Enter section name"
                        ...
                        <select name="school_year_id" id="school_year_id"
                            ...
                            @foreach($schoolYears ?? [] as $schoolYear)
                                <option value="{{ $schoolYear->id }}" {{ old('school_year_id') == $schoolYear->id ? 'selected' : '' }}>
                                    {{ $schoolYear->start_year }} - {{ $schoolYear->end_year }} @if($schoolYear->is_active) (Active) @endif
                                </option>
                            @endforeach
                        </select>
                    </div>
